...
cluster_map <- function(city){
	...
	tracts@data$cluster <- cutree(clusters,10)
	...
	to_plot %>% 
		ggplot(aes(x = long, y = lat, group = id, fill = cluster)) +
		geom_polygon() + 
...
make_cluster_groups <- function(city){
	tracts <- readOGR(dsn = paste0('data/cities/',city), layer = 'geo', verbose = FALSE)
	tracts <- tracts[sum(tracts@data[,columns]) != 0,]
	clusters <- readRDS(paste0('throughput/clusterings/', city))
	...
	I = mutual_info(tracts@data[,c(columns, 'cluster')])
	...
	tracts@data[,c(columns, 'cluster')] %>% 
		...
		arrange(desc(White)) %>% 
		mutate(cluster = factor(1:10, levels = 1:10)) %>%  
...
df <- rbind(make_cluster_groups('Detroit'), 
	  make_cluster_groups('Boston'),
	  make_cluster_groups('Dallas'),
	  make_cluster_groups('Atlanta')) 
