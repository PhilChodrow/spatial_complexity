...
method_illustration <- function(city, vmax){
	...
	p <- p +
		...
		scale_fill_continuous(low = 'white', high = 'steelblue', name= expression(J[Y](x)), limits = c(0, vmax)) +
...
first <- method_illustration('Orlando, FL', vmax = 1.5)
second <- method_illustration('Philadelphia, PA', vmax = 1.5)
...
cluster_map('Chicago, IL', nclusters = 10)
...
make_cluster_groups <- function(city, nclusters = 10){
	...
	tracts@data[,c(columns, 'cluster')] %>% 
		...
		arrange(desc(White)) %>% 
		mutate(cluster = factor(1:nrow(.), levels = 1:nrow(.))) %>%  
...
nclusters = 5
df <- rbind(make_cluster_groups('Detroit, MI', 3), 
	  make_cluster_groups('Boston, MA', 5),
	  make_cluster_groups('Chicago, IL', 4),
	  make_cluster_groups('Atlanta, GA', 2)) 
...
cities <- c('New Orleans, L','Boston, MA')
...
summary %>%
	...
	scale_color_continuous(low = 'white', high = 'firebrick',limits = c(0,.6)) +
	xlab('I(X,Y)') + 
	ylab('AUC')
...
summary %>%
	...
	scale_color_continuous(low = 'white', high = 'firebrick',limits = c(.3,.6), name = "J(X,Y)", guide = guide_colorbar(direction = 'horizontal')) +
