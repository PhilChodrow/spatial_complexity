...
g + theme(text=element_text(size=16, family="Palatino"))
...
cache %>%
	...
	geom_text_repel(data = highlight, aes(label = city), family = 'Palatino' ) + 
  theme(text=element_text(size=16, family="Palatino"),
...
cache %>% 
	...
	annotate('text', x = 300, y = .12, label = lm_eqn(cache), parse = T, family = 'Palatino') + 
	geom_point(color = 'grey') + 
	geom_point(color = 'firebrick', data = highlight) + 
	geom_text_repel(aes(label = city), size = 3, data = highlight, family = 'Palatino' ) +
...
ggsave('figs/example_cluster_map.pdf',  width = 7, height = 7)
...
make_cluster_groups('Detroit', 6) %>% 
  ...
  scale_fill_brewer(palette = 'Set3', name = NULL, guide = 'none', labels = LETTERS[1:nclusters]) + 
	...
	geom_text(aes(
		label = ifelse(p < 0.005, "", 100 * round(p,2))), size = 8, family = 'Palatino') + 
...
process_loss_curves <- function(loss_curves){
  loss_curves %>%
  	group_by(city) %>%
...
binary_loss_curves <- read_csv('throughput/loss_curves.csv') %>%
  group_by(city) %>% 
	arrange(desc(I_XY)) %>% 
	mutate(I_XY = max(I_XY) - I_XY,
		   nclust = row_number())

cities <- c('Atlanta','Philadelphia', 'Detroit')
...
summary <- compute_AUC(binary_loss_curves, adjust_thresh = 0.005) 
...
sub_curves %>% 
  ...
  theme(text=element_text(size=16, family="Palatino")) + 
	...
	geom_text(aes(x = 50, y = .2, label = paste('AUC = ', round(AUC,2))), data = sub_summary, family = 'Palatino') + 
...
ggsave('figs/AUC_illustration.pdf', width = 10, height = 4)
...
for(city in cities){
  ...
  if(city == 'Atlanta'){
    ...
    guide = guide_colorbar(barwidth = 1, barheight = 4, label.theme = element_text(size = 7, angle = 0, family = 'Palatino'))
  }
  ...
  tracts %>% 
    ...
    viridis::scale_fill_viridis(name = 'Percent Black', 
                                ...
                                limits = c(0,1),
                                guide = guide) + 
    ggtitle(paste0(city, ': J(X,Y) = ', J_XY)) + 
    ggthemes::theme_map() + 
