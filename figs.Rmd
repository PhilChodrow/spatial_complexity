...
races   <- c('Asian', 'Black', 'Other', 'White', 'Hispanic')
...
cluster_map('Detroit', 'Detroit', nclusters = 5)
...
adjust_thresh = 0.01
highlights  <- c('Philadelphia', 'Atlanta', 'Detroit')
...
end_points <- data.frame(city = c('Atlanta', 'Philadelphia', 'Detroit'),
                         nclust = c(50, 7, 8),
                         I_XY = c(.35, .175, .38))
...
```
```{r}
cache %>% filter(city %in% c('Philadelphia', 'Atlanta')) %>% 
  select(city, H_Y, I_XY, J_XY) %>%
  mutate_at(.cols = c('H_Y', 'I_XY','J_XY'), 
...
# check in here: are these binary clustering summaries? If so, we need to fix and get the full 
# clustering summaries instead. 
...
loss_curves %>% 
  group_by(city) %>% 
  filter(city %in% c('Atlanta', 'Philadelphia'),
         nclust %in% c(2, 5, 10)) %>% 
  mutate(I_XY = round(I_XY,2)) %>% 
  select(city, I_XY, nclust) %>% 
  spread(key = city, value = I_XY, fill = NA) %>%
  write_csv('figs/clustering_summary.csv')
  # slice(match(nclust, c('2', '5', '10', 'I(X,Y)')))

cache %>% 
  filter(city %in% c('Atlanta', 'Philadelphia')) %>% 
  select(city, I_XY) %>% 
  spread(key = city, value = I_XY, fill = NA) %>% 
  write_csv('figs/max_information.csv')
...
labels <- data_frame(label = c('A', 'B',  'E', 'C', 'D'),
                     long   = c(-83.35, -83.1568, -83.132, -82.905, -83.055),
                     lat  = c(42.37, 42.39, 42.32, 42.4, 42.4)) 
...
for(city in c('Chicago', 'Atlanta', 'Detroit', 'Philadelphia')){
  ...
  ggsave(out_dir, width = 14, height = height, units = 'cm')
  embed_fonts(out_dir, outfile=out_dir)
