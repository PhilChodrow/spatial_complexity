...
background %>% 
  ggplot(aes(x = nclust, y = I_XY)) +
  geom_path(aes(group = city), color = 'grey', alpha = 0.6) + 
  geom_path(aes(group = city, color = city), data = highlighted) +
  geom_text(aes(label = city), data = end_points) +
  theme_light() +
...
cluster_map <- function(map, city_name, races, nclusters){
  ...
  loss_curves <- readr::read_csv('throughput/loss_curves.csv') %>%
    ...
    for(ncluster in nclusters){
  		...
  		g <- g + geom_polygon(aes(x = long, y = lat, group = group),data = overlay,
  		                      alpha = 0, fill = NA, size = 5*weight, color = 'black') 
    }
  ...
  overlay <- unionSpatialPolygons(tracts, IDs = tracts@data$cluster) %>% fortify()
  g <- g + geom_polygon(aes(x = long, y = lat, group = group),data = overlay,
  		                      alpha = 0, fill = NA, size = 5*max_weight, color = 'white')
...
g +
  geom_point(aes(x = long, y = lat), data = labels, size = 6, pch=21, color = 'black', fill = 'white', alpha = .6) + 
  geom_text(aes(x = long, y = lat, label = label), 
                  data = labels, 
                  size = 4, fontface = 'bold')

out_dir <- paste0('figs/cluster_map_Detroit.pdf')
